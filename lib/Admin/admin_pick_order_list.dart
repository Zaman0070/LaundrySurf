...
class AdminPickOrderList extends StatefulWidget {
  int? index;
   AdminPickOrderList({Key? key,required this.index}) : super(key: key);

  @override
  State<AdminPickOrderList> createState() => _AdminPickOrderListState();
}

class _AdminPickOrderListState extends State<AdminPickOrderList> {
  String address='';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        title:  Text(widget.index==0?'Pickup Order List':"Ready to Deliver Order List",style: TextStyle(color: Colors.black,fontSize: 18.sp),),
        centerTitle: true,
        automaticallyImplyLeading: false,
      ),
      body:  FutureBuilder<QuerySnapshot>(
        future: services.order.where('orderStatus',isEqualTo: widget.index == 0?'RiderPickOrder':'AdminPickOrder')
            .orderBy('orderTime')
            .get(),
